...
def eval_move(board_copy, player, pos): # return the value of a move played by player
    ...
    neighbor_diameters = []
    for ne in ALL_NEIGHBOR[pos]:
        if (board_copy[ne] == player):
            visit = {pos:0 for pos in node_coordinates}
    ...
    if (new_diameter == 4 and max(neighbor_diameters < 4) and sum(neighbor_diameters) != 9): return 5
    if (new_diameter == 3 and max(neighbor_diameters < 3)): return 4
